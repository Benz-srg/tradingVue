...
export default (
  ...
  const UPDATE_TICK = (s) =>
    s.attr(
      'transform',
      `translate(${width - marginRight}, ${yScale(Y[total - 1])})`
    )

  const UPDATE_AREA_COLORS = (s) =>
    s
      .selectAll('stop')
      .data([
        {
  ...
  const pricetip = svg.append('g').style('pointer-events', 'none')

  const datetip = svg.append('g').style('pointer-events', 'none')
  ...
  const pointermoved = (event) => {
    ...
    tooltip.style('display', null).attr('transform', `translate(${sx}, ${sy})`)
    pricetip
      .style('display', null)
      .attr('transform', `translate(${sx}, ${sy})`)
    datetip.style('display', null).attr('transform', `translate(${sx}, ${sy})`)
    ...
    pricetip.selectAll('text').remove()
    pricetip.selectAll('rect').remove()

    datetip.selectAll('text').remove()
    datetip.selectAll('rect').remove()

    const pricerect = pricetip
      .selectAll('rect')
      .data([null])
      .join('rect')
      .attr('rx', 2)
    ...
    datex =
      sx + datex + datetipBox.width + ax > chartWidth
        ? chartWidth - sx - datetipBox.width - ax
        : sx > datex + datetipBox.width + ax ? datex : datetipBox.x - sx + ax

    datetext.attr('x', datex)
  ...
  const update = (tick) => {
    ...
    areaPath.transition().duration(duration).call(UPDATE_AREA)
    definedPath.transition().duration(duration).call(UPDATE_DEFINED)
    linePath.transition().duration(duration).call(UPDATE_LINE)

    live.call(UPDATE_TEXT).transition().duration(duration).call(UPDATE_TICK)
  }
