<template>
  <div class="d-flex mb-4">
    <ul
      class="d-tabs-list d-flex list-unstyled nav"
      role="tablist"
      v-on:click.capture="onClick"
    >
      <template v-for="option in intervals" v-bind:key="option">
        <li
          v-bind:class="'d-tabs' + (option === interval ? ' active' : '')"
          role="tab"
          aria-disabled="false"
          v-bind:aria-selected="option === interval ? 'true' : 'false'"
        >
          {{ option }}
        </li>
      </template>
  ...
  <div class="chart-aspect">
    ...
    <div v-else class="w-100 h-100 placeholder-glow position-absolute top-0">
      <div
        class="placeholder w-100 h-100 bg-opacity-10 bg-secondary rounded"
      ></div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    request () {
      this.loading = true
      binance
        .klines(this.symbol, this.interval)
        .then(this.onSuccess)
        .catch(console.log)
    },
    onClick (event) {
      ...
      if (
        ...
        this.intervals.indexOf(selection) === -1
      ) {
        return false
      }
  ...
  data: () => ({
    loading: false,
    fn: null,
    intervals: ['1m', '5m', '15m', '1h', '4h', '6h', '1d', '1w']
  })
}
</script>
