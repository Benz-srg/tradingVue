<template>
  <router-link
    v-bind:to="'/currencies/' + info.slug"
    v-bind:title="info.name + ' Charts'"
    v-bind:class="
      'pt-3 list-group-item list-group-item-action' +
      (isActive ? '' : ' disabled')
    "
    aria-current="true"
  >
    <div class="d-flex w-100 justify-content-between">
      <div class="d-flex flex-row align-items-center">
        <img
          ...
          v-bind:src="info.logo"
          width="24"
          height="24"
          v-bind:alt="info.name"
          loading="lazy"
        />
        <h5 class="mb-0 pair-meta-symbol">{{ info.symbol }}</h5>
      </div>
      <small>
    ...
    <div class="d-flex w-100 justify-content-between">
      ...
      <small class="pair-daily-change" v-bind:class="'fw-normal lh-sm text-end color-' + ticker.direction">
        <template v-if="!ticker.change">-</template>
        <template v-else>
         <span v-if="ticker.direction === 'down'"><BIconArrowDownShort /></span>
                <span v-else><BIconArrowUpShort /></span> {{ ticker.change }} %
        </template>
      </small>
    </div>
  </router-link>
